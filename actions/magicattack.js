window.game.actionData["magicattack"] = {
	priority:5,
	type:"magic",
	range:"melee",
	animationLength:49,
	actionEffect:function(user, target){
		if (target.dodging == false) {
			var damage_amount = Math.max(user.meleePower + user.magicPower - Math.max(target.meleeDefense + target.magicDefense - user.pierce, 0), user.pierce);
